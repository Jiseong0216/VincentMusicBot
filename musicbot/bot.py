...
class MusicBot(discord.Client):
    ...
    def _init_player(self, player, *, guild=None):
        player = player.on('플레이', self.on_player_play) \
                       .on('resume', self.on_player_resume) \
                       .on('pause', self.on_player_pause) \
                       .on('멈춰', self.on_player_stop) \
...
#######################################################################################################################
    ...
    async def cmd_play(self, message, player, channel, author, permissions, leftover_args, song_url):
        ...
        async with self.aiolocks[_func_() + ':' + str(author.id)]:
            ...
            if 'entries' in info:
                ...
                reply_text = self.str.get('cmd-play-playlist-reply', "**%s** songs 음악이 시작되었어요!")
            ...
            # If it's an entry
                ...
                reply_text = self.str.get('cmd-play-song-reply', "`%s` 라는 노래를 유XX 에서 찾아 틀어 드렸습니다!")
                btext = entry.title
    ...
    async def cmd_skip(self, player, channel, author, message, permissions, voice_channel, param=''):
        ...
        if skips_remaining <= 0:
            ...
            return Response(
                self.str.get('cmd-skip-reply-skipped-1', '`{0}` 을 스킵하셨습니다.{1}').format(
                    current_entry.title,
